...
confHelp = renderText 80
    ...
    , ""
    , "In every file, you can prefix lines with \"<a>*<b>:\", so that they will only"
    , "apply to files with file names starting with <a> and ending with <b>. Contrary"
    , "to common use, you must use exactly one '*' per directive. If the prefix starts"
    , "with a '/', the pattern will be matched against the absolute file path, if not,"
    , "the path relative to the current working directory will be used. Note that this"
    , "does include empty prefixes, so if you want to match a suffix against the"
    , "absolute path, you should use \"/*some/directory/file.ending\"."
    , ""
