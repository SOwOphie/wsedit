...
usageHelp = renderText 80 $
    ...
    , "                editor on older systems, as it seems to be quite the resource"
    , "                hog."
    ...
    , "                and directories under the current working directory, skipping"
    , "                hidden ones (starting with a dot). Every file with the same file"
    , "                ending as the opened file will be read, and a dictionary will be"
    , "                built from all words from lines at depth n (either n tabs or"
    , "                n*tabWidth spaces). This dictionary will then be used to feed"
    , "                the autocomplete function. The scan will take place everytime"
    , "                you save or load."
    , "                SETTING THIS GLOBALLY WILL MAKE YOUR EDITOR TAKE AGES TO START"
    , "                UP, E.G. WHEN RUNNING FROM THE HOME DIRECTORY!"
    , ""
    , ""
    , ""
